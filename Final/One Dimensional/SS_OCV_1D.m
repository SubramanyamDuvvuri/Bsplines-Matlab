...
knotspan=knot_calculation (nSensors,Start_point,End_point); %Automatic Calculation of Knot Span --> Rupert Extimation min(n/4,40)
    ...
    for lambda_counter =1:length(lambda)
        ...
        if lambda_counter > 1
            if RMS(lambda_counter-1)< RMS(lambda_counter)
                ...
                break;
                figure(5)
                plot (lambda(1:lambda_counter),RMS(1:lambda_counter),'-','LineWidth',3)
