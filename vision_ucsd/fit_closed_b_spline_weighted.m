%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% coefs = fit_closed_b_spline_weighted(s,x,w,L)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This function fits a closed B-spline with L knots to the input
% data (s,x) with weights w, and returns the control points.  The
% B-spline chosen is optimal in the least-squares sense.  The
% control points coefs chosen minimize 
% \sum_{i=1}^N w_i trace((x_i - f(s_i ; coefs)') * (x_i - f(s_i ; coefs))).  
%
% Input:
...
% w: An array of size N x 1, indicating the weight of each data
% point.  
%
% Output:
...
% w_i trace((x_i - f(s_i ; coefs)') * (x_i - f(s_i ; coefs)))
% = trace((x - A * coefs)'*W*(x-A*coefs)), where W = diag(w).  
% We can take the derivative w.r.t. coefs and set it to 0:
% zeros(L,ndims) = - 2 * A' * W * (x - A * coefs)
% A' * W * x = A' * W * A * coefs
% inv(A'*W*A) * A' * W * x = coefs
%
...
%
function [coefs,A] = fit_closed_b_spline_weighted(s,x,w,L)
...
% find coefs that minimizes trace((x - f(s))'*W*(x - f(s)))
% = trace((x - A * coefs)'*W*(x - A * coefs)).
